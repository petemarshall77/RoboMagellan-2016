...
# 
import math, serial, signal, time, sys, os.path
...
def GPS_thread():
  ...
  global GPS_run_flag
  GPS_run_flag = True
  GPS_samples = deque([(0,0)], maxlen = 10)
    ...
    if GPSSerial.inWaiting() > 0:       
      ...
      if GPS_data[0:6]=="$GPRMC":
        ...
        if GPS_fields[2]=='A':
          ...
          GPS_samples.append((GPS_Lat, GPS_Long))
          GPS_Avg_Lat = 0
          GPS_Avg_Long = 0
          for sample in GPS_samples:
            GPS_Avg_Lat += sample[0]
            GPS_Avg_Long += sample[1]
          GPS_Avg_Lat /= len(GPS_samples)
          GPS_Avg_Long /= len(GPS_samples)
        # No data available, set values to zero  
        else:
          GPS_Lat=0.0
          GPS_Long=0.0 
...
def get_Avg_GPS():
  global GPS_Avg_Lat
  global GPS_Avg_Long
  return (GPS_Avg_Lat,GPS_Avg_Long)
    ...
    while found_it == False:
     ...
     while currentDistance < 5:
        ...
        if get_camera_values()[0] == 0:
          ...
          steer_value = ((get_camera_values()[0] * (500/160))-500) 
...
#========================================================
# start_at - calculate GPS deviation at start
#========================================================
def start_at(start_Lat, start_Long):
  (avg_Lat, avg_Long) = get_Avg_GPS()
  (distance, bearing) = get_distance_and_bearing(start_Lat, start_Long, avg_Lat, avg_Long)
  print "GPS reading is", distance, "meters,", bearing, "degrees from indicated start"
  return (avg_Lat - start_Lat, avg_Long - start_long)
...
def march29course():
  (delta_Lat, delta_Long) = start_at(33.77823333333333, 118.41887666666666)
  drive_to_cone(1600, 33.778195 + delta_Lat, 118.41895 + delta_Long)
  drive_to_cone(1600, 33.778365 + delta_Lat, 118.41914833333334 + delta_Long)
  drive_to_cone(1600, 33.778623333333336 + delta_Lat, 118.41893 + delta_Long)
  drive_to(1600, 33.778435 + delta_Lat, 118.419045 + delta_Long)
  drive_to(1600, 33.77834 + delta_Lat, 118.41883166666666 + delta_Long)
